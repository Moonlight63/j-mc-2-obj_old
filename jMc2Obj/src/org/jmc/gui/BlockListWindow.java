...
public class BlockListWindow extends JFrame{
	...
	public BlockListWindow(){
		...
		mainPanel.removeAll();
		mainPanel.add(scrollList, BorderLayout.NORTH);
		mainPanel.add(searchbar, BorderLayout.SOUTH);
		mainPanel.add(selectAll);
		mainPanel.add(deselectAll);
		mainPanel.add(toggleselect);
		...
		toggleselect.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				for(int i = 0; i < labelList.getModel().getSize(); i++){
					labelList.getModel().getElementAt(i).setSelected(!labelList.getModel().getElementAt(i).isSelected());
				}
				saveList("CurSelection");
				revalidate();
				repaint();
	...
	public void updateListings(){
		...
		for(CheckListItem item : listItems){
			if(((BlockInfo) item.getItem()).getName().toLowerCase().contains(searchbar.getText().toLowerCase())){
				dlm.addElement(item);
	...
	public void saveList(String group){
		...
		try {
			...
			for(int i = 0; i < labelList.getModel().getSize(); i++){
				...
				BlockInfo labelElement = (BlockInfo) labelList.getModel().getElementAt(i).getItem();
				TAG_Byte byteElement = (TAG_Byte) presetgroup.getElement(labelElement.getName());
				
			    if(byteElement != null)
			    	byteElement.value = labelList.getModel().getElementAt(i).isSelected() ? (byte)1 : (byte)0;
			    else
			    	presetgroup.elements.add(new TAG_Byte(labelElement.getName(), labelList.getModel().getElementAt(i).isSelected() ? (byte)1 : (byte)0));
	...
	public Set<BlockInfo> getAllSelectedItems(){
		...
		for (int i = 0; i < labelList.getModel().getSize(); i++){
			if(((CheckListItem)labelList.getModel().getElementAt(i)).isSelected()){
				tempArray.add((BlockInfo) labelList.getModel().getElementAt(i).getItem());
			}
		}
		return tempArray;
	}
	...
	private void selectItem(Point point){
	    ...
	    if (index >= 0){
	      CheckListItem item = (CheckListItem)labelList.getModel().getElementAt(index);
